...
> * 在没有设置偏移量的时候，他是显示在默认位置上不动，但是会出现层及变化；
> * 当给绝对定位元素添加了偏移量之后，他就相对**可视窗口的第一屏**显示偏移量了；也就可一理解可视窗口的第一屏是绝对定位元素的**参照物（包含块）**；
...
> * 绝对定位元素，会相对离他**最近的**并且**具有定位属性**的**祖先元素**进行绝对定位；
...
>   * 绝对定位（absolute）：参照物是包含块（是具有定位属性的父元素，默认时可视窗口的第一屏是一个大的包含块，绝对定位元素都会相对可视窗口的第一屏进行绝对定位）
