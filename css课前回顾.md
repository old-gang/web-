...
H5语义化标签：
结构标签
    header   表示整个页面或者某一个内容的标题区
    footer   表示整个页面或者某一个内容的尾部区
    nav      表示导航链接
    main     表示主体内容区（ie不兼容）
    section  表示一个区块，类似于div；
    article  表示与上下文无关的内容区
    aside    作为article的一个辅助区域
    figure   表示一块独立区域
    figcaption 表示figure标题标签，可以写在figure内部的第一个或者最后一个子元素的位置上；
    mark     让文本高亮显示，类似于span标签 ，是行内元素
    hgroup   标题组
多媒体标签
    视频：
        语法：
    ...
    多媒体标签的属性：
         loop      循环播放
         controls  添加媒体控件
         autoplay  自动播放，需要和muted一起使用
         muted     静音播放   
         
         poster="路径"  设置封面，在播放后消失；（视频的属性）
