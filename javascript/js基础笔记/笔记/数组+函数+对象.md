...
正则表达式(regular expression)描述了一种字符串匹配的模式，可以用来检查一个字符串是否含有某种子串、将匹配的子串做替换或者从某个字符串中取出符合某个条件的子串等。说白了**正则表达式就是处理字符串的,我们可以用它来处理一些复杂的字符串**。更多的时候我们使用正则的意义在于**判断某字符串是否符合某个规则**，这个规则就是正则表达式
...
> 注：默认从字符串的第1位开始匹配，如果不是则继续向下匹配，直到找到匹配项，从头到尾如果都没有匹配项则返回为false
...
该函数返回一个数组，其中存放匹配的结果，有则返回匹配项，没有匹配项返回null;

```js
let result2 = /e/.exec("The best things in life are free!")
console.log(result2);
```
...
一般限制数量，都会使用？；+ * 都是贪婪的         
      ...
      示例二 ： 源字符串：aa<div>test1</div>bb<div>test2</div>cc
          ...
          这里采用的是贪婪模式。在匹配到第一个'<div>'已经可以使整个表达式匹配成		  功，但是由于采用的是贪婪模式，所以仍然要向右尝试匹配，
